    ...
    return (
        <Container>
            ...
            <Row className="mb-3 d-flex justify-content-between align-items-center">
                ...
                <Col className="text-end">
                    <Button variant="primary" onClick={() => {
                        dsluong.map((data) => {
                            axios.post("http://localhost:8080/getphieuluongpdf/getphieuluongpdf", data, {
                                responseType: 'blob',
                            })
                                .then(response => {
                    ...
                    <Button style={{ marginLeft: "30px" }} variant="primary" onClick={() => {
                        handleModalOpen5()
            ...
            <Modal show={showModal} onHide={handleModalClose} centered size="lg">
                <Modal.Body style={{ width: "80%", height: "70%", margin: "0 auto", backgroundColor: "#f0f0f0" }}>
                    <h3 style={{ textAlign: "center" }}>Phiếu Lương</h3>
                    <hr></hr>
                    <div style={{ display: "flex" }}>
                        <div style={{ width: "100%" }}>
                            <p style={{ textAlign: "center" }}>Thời gian lập {ngayBatDau} - {ngayKetThuc}</p>
                        </div>
                    </div>
                    <div style={{ display: "flex" }}>
                        <div style={{ width: "50%" }}>
                            ...
                            <hr></hr>
                            <p style={{ textAlign: "center" }}>Tên nhân viên: {dongduocchon.current?.thongtinnhanvien?.nv_HOTEN}</p>
                            <p style={{ textAlign: "center" }}>Số điện thoại: {dongduocchon.current?.thongtinnhanvien?.nv_SDT}</p>
                            <p style={{ textAlign: "center" }}>Giới tính: {dongduocchon.current?.thongtinnhanvien?.nv_GIOITINH ? "Nam" : "Nữ"}</p>
                            <p style={{ textAlign: "center" }}>email: {dongduocchon.current?.thongtinnhanvien?.nv_EMAIL}</p>
                        </div>
                        <div style={{ width: "50%" }}>
                            <p style={{ textAlign: "center" }}>Thông tin lương cơ bản</p>
                            <hr></hr>
                            <p style={{ textAlign: "center" }}>Lương cơ sở: {dongduocchon.current?.luongcoban}</p>
                            <p style={{ textAlign: "center" }}>Lương tăng ca: {dongduocchon.current?.luongtangca}</p>
                            <p style={{ textAlign: "center" }}>Khoản khấu trừ: {dongduocchon.current?.tongkhautru}</p>
                            <p style={{ textAlign: "center" }}>Lương thực nhận: {dongduocchon.current?.luongnhan}</p>
                        </div>
                    </div>
                ...
                <Button onClick={() => {
                    axios.post("http://localhost:8080/getphieuluongpdf/getphieuluongpdf", dongduocchon.current, {
                        responseType: 'blob',
                    })
                        .then(response => {
                            const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }));
                            const link = document.createElement('a');
                            link.href = url;
                            link.setAttribute('download', `NV${dongduocchon.current?.thongtinnhanvien?.nv_ID}.pdf`);
                            document.body.appendChild(link);
                            link.click();
                        })
                        .catch(error => {
                ...
                }} variant="contained">Xuất phiếu lương</Button>
            </Modal>
            ...
            <Modal show={showModal5} onHide={handleModalClose5} centered size="lg">
                ...
                <Button style={{ backgroundColor: "#0d6efd", color: "white" }} onClick={() => {
                    dsluong.forEach((f) => {
                        ...
                        axios.post("http://localhost:8080/getphieuluongpdf/sendemailphieuluong", form)
                    })
                    alert("Gửi thông tin thành công")

                }} variant="contained">Gửi thông tin hàng loạt</Button>
            </Modal>
