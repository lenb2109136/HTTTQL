...
const MyComplaintsPage = () => {
  ...
  const nvId = localStorage.getItem('userId'); // Thay 'userId' bằng key bạn dùng
  ...
  const fetchMyComplaints = async () => {
    try {
      ...
      const myComplaints = response.data.filter(
        complaint => complaint.nv_ID.NV_ID === parseInt(nvId)
      );
        ...
        <Modal.Body>
          <Form onSubmit={handleSubmit}>
            <Form.Group controlId="formNV_ID">
              ...
              <Form.Control
                type="number"
                value={formData.nv_ID || nvId || ''} // Sử dụng nvId từ localStorage nếu có
                onChange={e =>
                  setFormData({ ...formData, nv_ID: e.target.value })
                }
                required
                disabled={!!nvId} // Vô hiệu hóa nếu nvId từ localStorage đã có
              />
            </Form.Group>
