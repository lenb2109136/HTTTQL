...
const EmployeeSalaryAdvance = () => {
  ...
  const [nvId, setNvId] = useState(null); // Đổi giá trị khởi tạo thành null để dễ kiểm tra
  ...
  useEffect(() => {
    const loadEmployeeData = () => {
      ...
      console.log('Raw employee data from localStorage:', employeeDataString); // Log dữ liệu thô
      ...
      try {
        ...
        console.log('Parsed employee data:', employeeData); // Log dữ liệu đã parse
        ...
        if (!id || id === 0) {
          ...
          localStorage.removeItem('employee'); // Xóa dữ liệu không hợp lệ
          navigate('/login');
          return;
        }
        setNvId(id);
      ...
      if (!ws.current || ws.current.readyState === WebSocket.CLOSED) {
        ...
        ws.current.onopen = () => {
          ...
          if (nvId) {
            ...
            showToast(
              'error',
              'Không thể kết nối WebSocket: NV_ID không hợp lệ'
            );
          }
        };
