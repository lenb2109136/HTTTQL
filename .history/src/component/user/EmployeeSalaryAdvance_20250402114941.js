...
const EmployeeSalaryAdvance = () => {
  ...
  // State để lưu trữ các thông báo đã hiển thị
    ...
    const connectWebSocket = () => {
      if (!ws.current || ws.current.readyState === WebSocket.CLOSED) {
        ...
        ws.current.onopen = () => {
          ...
          toast.success('Kết nối WebSocket thành công!'); // Thông báo khi kết nối thành công
        };
        ...
        ws.current.onmessage = event => {
          ...
          try {
            ...
            if (data.type === 'statusUpdate' && data.nvId === nvId) {
              // Tạo message để kiểm tra trùng lặp
              ...
              if (!isDuplicate) {
                // Thêm thông báo vào danh sách đã hiển thị
                setDisplayedNotifications(prev => [
                  ...prev,
                  { message, time: data.time },
                ]);
                toast.success(message); // Hiển thị thông báo duy nhất 1 lần
