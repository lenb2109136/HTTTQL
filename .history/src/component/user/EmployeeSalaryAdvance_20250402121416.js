...
const EmployeeSalaryAdvance = () => {
  ...
  try {
    employeeData = employeeDataString ? JSON.parse(employeeDataString) : {};
    nvId = employeeData.NV_ID || 0;
    console.log('Employee data from localStorage:', employeeData);
    console.log('Extracted NV_ID:', nvId);
  } catch (error) {
  ...
  useEffect(() => {
    const connectWebSocket = () => {
      if (!ws.current || ws.current.readyState === WebSocket.CLOSED) {
        ...
        ws.current.onopen = () => {
          ...
          if (nvId) {
            ws.current.send(`subscribe:${nvId}`); // Gửi subscription chỉ khi có nvId hợp lệ
    ...
    if (nvId) {
      // Chỉ kết nối WebSocket nếu nvId hợp lệ
      connectWebSocket();
    }
