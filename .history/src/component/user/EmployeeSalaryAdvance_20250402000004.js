...
const EmployeeSalaryAdvance = () => {
  ...
  const nvId = employeeData.NV_ID || 0; // Lấy NV_ID từ localStorage
  const ws = new WebSocket('ws://localhost:8080/hethongquanly/websocket');

  useEffect(() => {
    console.log('NV_ID từ localStorage:', nvId); // Debug NV_ID
    fetchSalaryAdvances();
    setupWebSocket();
  ...
  const setupWebSocket = () => {
    ws.onopen = () => {
      ...
      ws.send(JSON.stringify({ type: 'subscribe', nvId }));
    };

    ws.onmessage = event => {
      console.log('WebSocket message:', event.data); // Debug tin nhắn
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (Array.isArray(data)) {
          setSalaryAdvances(data.filter(item => item.nv_ID === nvId)); // Lọc lại phía client nếu cần
        } else {
          setSalaryAdvances([]);
        ...
        toast.error(`Lỗi khi lấy danh sách ứng lương: ${response.status}`);
        setSalaryAdvances([]);
      }
    } catch (error) {
