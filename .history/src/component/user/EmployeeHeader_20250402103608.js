...
const EmployeeHeader = () => {
  ...
  const ws = useRef(null); // Sử dụng useRef để quản lý WebSocket instance duy nhất
      ...
      ws.current.onmessage = event => {
        ...
        try {
          ...
          if (data.type === 'statusUpdate' && data.nvId === employee.NV_ID) {
            // Kiểm tra trùng lặp dựa trên message và time
            ...
            if (!isDuplicate) {
              ...
              toast.info(data.message); // Hiển thị toast thông báo
            } else {
  ...
  }, [employee.NV_ID]); // Chỉ chạy khi NV_ID thay đổi

  // Xử lý khi nhấp vào thông báo
  const handleNotificationClick = notification => {
    // Chuyển hướng đến trang EmployeeSalaryAdvance
