...
const EmployeeHeader = () => {
  ...
  // Quản lý danh sách thông báo động
  const [notifications, setNotifications] = useState([]);
  const [displayedToastMessages, setDisplayedToastMessages] = useState([]); // State để lưu trữ các message đã hiển thị toast
  ...
  useEffect(() => {
    if (!ws.current || ws.current.readyState === WebSocket.CLOSED) {
      ...
      ws.current.onopen = () => {
        ...
        if (employee.NV_ID) {
          ws.current.send(`subscribe:${employee.NV_ID}`); // Subscribe với NV_ID
        }
      };

      ws.current.onmessage = event => {
        console.log('WebSocket message in Header:', event.data);
        try {
          const data = JSON.parse(event.data);
          if (data.type === 'statusUpdate' && data.nvId === employee.NV_ID) {
            // Kiểm tra trùng lặp dựa trên message và time cho danh sách thông báo
            const isDuplicateNotification = notifications.some(
            ...
            if (!isDuplicateNotification) {
              ...
              setNotifications(prev => [newNotification, ...prev]); // Thêm thông báo mới vào danh sách

              // Kiểm tra trùng lặp dựa trên message (bỏ qua time) cho toast
              const isDuplicateToast = displayedToastMessages.includes(
                data.message
              );
              if (
                !isDuplicateToast &&
                data.message.toLowerCase().includes('lương')
              ) {
                setDisplayedToastMessages(prev => [...prev, data.message]); // Lưu message đã hiển thị toast
                toast.info(data.message, {
                  className: 'teal-toast', // Áp dụng màu xanh nước biển
                });
              } else if (isDuplicateToast) {
                console.log('Duplicate toast skipped:', data.message);
              }
            } else {
              console.log('Duplicate notification skipped:', data.message);
            }
          }
        } catch (error) {
  ...
  const validateProfileForm = () => {
    ...
    if (!profileFormData.NV_EMAIL) {
      newErrors.NV_EMAIL = 'Email là bắt buộc';
    } else if (!/\S+@\S+\.\S+/.test(profileFormData.NV_EMAIL)) {
  ...
  const handleProfileSubmit = async e => {
    ...
    try {
      const payload = {
        ...
        NV_EMAIL: profileFormData.NV_EMAIL,
      };
            ...
            <Row>
              <Col md={6}>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    type="email"
                    value={profileFormData.NV_EMAIL}
                    onChange={e =>
                      setProfileFormData({
                        ...profileFormData,
                        NV_EMAIL: e.target.value,
                      })
                    }
                    isInvalid={!!profileErrors.NV_EMAIL}
                  />
                  <Form.Control.Feedback type="invalid">
                    {profileErrors.NV_EMAIL}
                  </Form.Control.Feedback>
                </Form.Group>
              </Col>
              <Col md={6}>
