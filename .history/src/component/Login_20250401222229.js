...
const LoginPage = () => {
  ...
  const [isSuccessNotified, setIsSuccessNotified] = useState(false); // Thêm trạng thái để kiểm soát thông báo
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (response.data && response.data.NV_ID) {
        ...
        // Chỉ hiển thị thông báo nếu chưa hiển thị trước đó
        if (!isSuccessNotified) {
          toast.success('Đăng nhập thành công!', {
            position: 'top-right',
            autoClose: 2000,
          });
          setIsSuccessNotified(true); // Đánh dấu đã hiển thị thông báo
        }
