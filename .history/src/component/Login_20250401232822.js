...
const LoginPage = () => {
  ...
  // Reset isSuccessNotified when the component mounts
  useEffect(() => {
    // This runs when the component is mounted or re-mounted
    setIsSuccessNotified(false);
  }, []); // Empty dependency array ensures it runs only on mount

  const [isSuccessNotified, setIsSuccessNotified] = useState(false); // State to control toast
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (response.data && response.data.NV_ID) {
        ...
        // Hiển thị thông báo thành công chỉ nếu chưa hiển thị
        if (!isSuccessNotified) {
          toast.success('Đăng nhập thành công!', {
            position: 'top-right',
            autoClose: 2000,
          });
          setIsSuccessNotified(true); // Đánh dấu đã hiển thị
        }

        // Điều hướng ngay lập tức về /userhome
