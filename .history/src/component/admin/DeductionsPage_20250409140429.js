...
const EmployeesPage = () => {
  ...
  const handleSubmit = e => {
    e.preventDefault();
    axios
      .post('http://localhost:8080/khautru/add', newDeduction)
      .then(response => {
  ...
  const [dulieu, setDulieu] = useState([
    {
      KT_LOAITIENKHAUTRU: 'Bảo hiểm',
      KT_DIENGIAI: 'Khấu trừ bảo hiểm xã hội',
      KT_SOTIEN: 500000.0,
    },
    {
      KT_DIENGIAI: 'Khấu trừ thuế TNCN',
      KT_SOTIEN: 25000.0,
      KT_LOAITIENKHAUTRU: 'Mức thuế',
    },
    {
      KT_DIENGIAI: 'Khấu trừ bảo hiểm y tế',
      KT_LOAITIENKHAUTRU: 'Bảo hiểm',
      KT_SOTIEN: 200000.0,
    },
    {
      KT_LOAITIENKHAUTRU: 'Công Ty',
      KT_DIENGIAI: 'Khấu trừ thiệt hai công',
      KT_SOTIEN: 200.0,
    },
    {
      KT_DIENGIAI: 'Khấu trừ phí phục vụ',
      KT_SOTIEN: 500.0,
      KT_LOAITIENKHAUTRU: 'khấu trừ công ty',
    },
    {
      KT_DIENGIAI: 'Khấu trừ sinh hoạt phí',
      KT_LOAITIENKHAUTRU: 'khấu trừ công ty',
      KT_SOTIEN: 200.0,
    },
    {
    ...
    },
    {
      ...
      KT_LOAITIENKHAUTRU: 'Công Ty',
      KT_SOTIEN: 200.0,
      KT_DIENGIAI: 'Đồng phục hàng năm',
    },
  ]);
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:8080/khautru/${
          not.current == false ? 'thongke' : 'thongkenot'
        }?nbd=${formattedStartDate}&nkt=${formattedEndDate}`
      )
      .then(response => {
          ...
          <div className="d-flex gap-3 mb-3">
            <Form.Select
              ...
              onChange={e => {
                ...
                axios
                  .get(
                    `http://localhost:8080/api/nhanvien/getPhongBanSoDienThoai?idphongban=${e.target.value}&sodienthoai=${soDienThoai}`
                  )
                  .then(data => {
                    setdsnhanvien(data.data.data);
                  });
            ...
            <Form.Control
              ...
              onChange={e => {
                ...
                axios
                  .get(
                    `http://localhost:8080/api/nhanvien/getPhongBanSoDienThoai?idphongban=${selectedPhongBan}&sodienthoai=${e.target.value}`
                  )
                  .then(data => {
                    setdsnhanvien(data.data.data);
                  });
