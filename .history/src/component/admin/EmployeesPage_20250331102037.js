...
const EmployeesPage = () => {
  ...
  const handleEdit = employee => {
    ...
    setFormData({
      ...
      PB_ID: employee.PB_ID?.PB_ID.toString() || '',
      NV_NGAYSINH: employee.NV_NGAYSINH,
      NV_GIOITINH: employee.NV_GIOITINH === 1, // Chuyển 1 thành true (Nam), 0 thành false (Nữ)
      NV_EMAIL: employee.NV_EMAIL,
      NV_SDT: employee.NV_SDT,
      NV_USERNAME: employee.NV_USERNAME,
      NV_PASSWORD: '', // Không hiển thị mật khẩu cũ để bảo mật
          ...
          {employees.length === 0 ? (
            ...
            employees.map((emp, index) => (
              <tr key={emp.NV_ID}>
                <td>{index + 1}</td>
                <td>{emp.NV_HOTEN}</td>
                <td>{emp.PB_ID.PB_TEN}</td>{' '}
                {/* Loại bỏ optional chaining vì PB_ID luôn có */}
                <td>{emp.NV_SDT}</td>
                <td>
                  <Button
                    variant="warning"
