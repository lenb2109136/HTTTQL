...
const AdvanceSalaryPage = () => {
  ...
  const [deleteId, setDeleteId] = useState(null); // ID của yêu cầu cần xóa
  ...
  const validateForm = () => {
    ...
    } else if (parseFloat(formData.ul_TIEN) <= 1000) { // Sửa thành lớn hơn 1000
      newErrors.ul_TIEN = 'Số tiền ứng lương phải lớn hơn 1000 VNĐ';
    }
    if (!formData.ul_NGAYUL) {
      newErrors.ul_NGAYUL = 'Vui lòng chọn ngày ứng lương';
    } else if (!isEditMode) {
      // Chỉ validate ngày khi tạo mới
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      let response;
      if (isEditMode) {
        response = await axios.put(`${API_URL}/ung-luong/${formData.ul_ID}`, payload);
      ...
      toast.error('Lỗi khi lưu yêu cầu: ' + (error.response?.data || error.message));
    }
  };
  ...
  const handleReject = async id => {
    try {
      ...
      toast.error('Lỗi khi từ chối: ' + (error.response?.data || error.message));
    }
  };
            ...
            <Row>
              <Col md={6}>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    type="number"
                    min="1000" // Thay đổi min thành 1000
                    step="1000"
                    ...
                    onChange={e => {
                      ...
                      setFormData({
                        ...formData,
                        ul_TIEN: value > 1000 ? value : '', // Chỉ chấp nhận lớn hơn 1000
                      });
                    }}
