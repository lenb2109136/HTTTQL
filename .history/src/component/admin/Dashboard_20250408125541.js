...
const AdminDashboard = () => {
  const [monthlySalaryData, setMonthlySalaryData] = useState(null); // Biểu đồ lương 12 tháng
  ...
  const fetchChartData = async () => {
    try {
      ...
      // 1. Monthly Salary (Bar Chart - 12 tháng)
      const monthlySalary = Array(12).fill(0); // Khởi tạo mảng 12 tháng với giá trị 0
      phieuLuongData.forEach(item => {
        const month = new Date(item.ngayPhat).getMonth(); // Lấy chỉ số tháng (0-11)
      ...
      // 4. Advance Trend (Line Chart)
      const monthlyAdvance = ungLuongData.reduce((acc, item) => {
        const date = new Date(item.ul_NGAYUL).toLocaleString('default', {
          month: 'short',
          year: 'numeric',
        });
        acc[date] = (acc[date] || 0) + (item.ul_TIEN || 0);
        return acc;
      }, {});
      setAdvanceTrendData({
        labels: Object.keys(monthlyAdvance),
        datasets: [
          {
            label: 'Tổng ứng lương',
            data: Object.values(monthlyAdvance),
      ...
      // 5. Employee Count by Department (Doughnut Chart)
      ...
      // 6. Salary vs Deduction by Department (Grouped Bar Chart)
    ...
    <Container fluid className="p-4 bg-light min-vh-100">
      ...
      <Row className="mb-5 g-3">
        {[
          ...
          <Col md={2} key={index}>
            <Card
              className={`stat-card shadow-sm rounded-3 border-0 ${stat.className}`}
            >
      ...
      {/* Charts */}
      <Row className="g-4">
        {/* Biểu đồ lương 12 tháng - Hàng đầu */}
        <Col md={12}>
