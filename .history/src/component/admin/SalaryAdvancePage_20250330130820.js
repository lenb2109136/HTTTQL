...
const AdvanceSalaryPage = () => {
  ...
  const fetchAdvanceRequests = async () => {
    try {
      ...
      setErrorMessage('Không thể tải danh sách ứng lương: ' + error.message);
    }
  };
  ...
  const fetchEmployees = async () => {
    try {
      ...
      setErrorMessage('Không thể tải danh sách nhân viên: ' + error.message);
    }
  };

  const getEmployeeName = nvId => {
    const employee = employees.find(emp => emp.nv_ID === nvId);
    return employee ? employee.nv_HOTEN : 'N/A';
  };
  ...
  const validateForm = () => {
    ...
    } else if (parseFloat(formData.ul_TIEN) <= 0) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        setSuccessMessage('Đã sửa yêu cầu ứng lương thành công!');
      } else {
        ...
        setSuccessMessage('Đã tạo yêu cầu ứng lương thành công!');
      }
  ...
  const handleApprove = async id => {
    try {
      ...
      setSuccessMessage('Đã duyệt yêu cầu ứng lương thành công!');
      fetchAdvanceRequests();
    } catch (error) {
      console.error('Error approving advance request:', error);
      setErrorMessage(
        'Lỗi khi duyệt: ' + (error.response?.data || error.message)
      );
    }
  };
  ...
  const handleReject = async id => {
    try {
      ...
      setSuccessMessage('Đã từ chối yêu cầu ứng lương thành công!');
      fetchAdvanceRequests();
    } catch (error) {
  ...
  const handleDelete = async id => {
    const firstConfirm = window.confirm(
      'Bạn có chắc chắn muốn xóa yêu cầu ứng lương này không?'
    );
    if (firstConfirm) {
      const secondConfirm = window.confirm(
        'Hành động này không thể hoàn tác. Bạn có thực sự muốn xóa không?'
      );
      if (secondConfirm) {
        try {
          setErrorMessage('');
          ...
          console.log('Đã xóa ứng lương ID:', id);
          setSuccessMessage('Đã xóa yêu cầu ứng lương thành công!');
          fetchAdvanceRequests();
        } catch (error) {
          console.error('Error deleting advance request:', error);
          setErrorMessage(
            'Lỗi khi xóa: ' + (error.response?.data || error.message)
          );
        }
      }
    }
