...
const AdminDashboard = () => {
  ...
  const fetchChartData = async () => {
    ...
    try {
      ...
      const salaryData = salaryJson.data || []; // Nếu không có data, trả về mảng rỗng
      const totalSalaryConfig = {
        labels:
          salaryData.length > 0
            ? salaryData.map(item => item.departmentName || 'Không xác định')
            : ['Không có dữ liệu'],
        datasets: [
          {
            ...
            data:
              salaryData.length > 0
                ? salaryData.map(item => item.totalSalary || 0)
                : [0],
      ...
      const filteredUngLuong = ungLuongData.filter(item => {
        const advanceDate = new Date(item.ulNgayul); // Giả sử trường là ulNgayul từ bảng ung_luong
        return (
          advanceDate >= new Date(startDate) && advanceDate <= new Date(endDate)
        );
      });
      const monthlyData = filteredUngLuong.reduce((acc, item) => {
        const date = new Date(item.ulNgayul).toLocaleString('default', {
          month: 'short',
          year: 'numeric',
        });
        acc[date] = (acc[date] || 0) + (item.ulTien || 0); // ulTien từ bảng ung_luong
        return acc;
      }, {});
