...
const EmployeesPage = () => {
  ...
  const confirmDelete = async () => {
    try {
      ...
      console.error('Chi tiết lỗi khi xóa nhân viên:', error.response);
      const errorMessage = error.response?.data || error.message;

      // Phân tích lỗi khóa ngoại để xác định bảng gây lỗi
      if (errorMessage.includes('foreign key constraint fails')) {
        const tableMatch = errorMessage.match(/`([^`]+)`.`([^`]+)`/);
        const tableName = tableMatch ? tableMatch[2] : 'không xác định';
        toast.error(
          `Không thể xóa nhân viên vì dữ liệu đang được tham chiếu trong bảng: ${tableName}`
        );
      } else {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const payload = {
        ...formData,
        nv_GIOITINH: formData.nv_GIOITINH ? 1 : 0, // Chuyển true/false thành 1/0
        pb_ID: parseInt(formData.pb_ID), // Chỉ gửi pb_ID dưới dạng số
      };
