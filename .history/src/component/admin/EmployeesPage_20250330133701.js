...
const EmployeesPage = () => {
  ...
  useEffect(() => {
    fetchEmployees();
    fetchDepartments();
    toast.success('Test toast'); // Kiểm tra ToastContainer
  }, []);
  ...
  const confirmDelete = async () => {
    try {
      console.log('Deleting employee ID:', deleteId);
      ...
      toast.error(
        'Lỗi khi xóa nhân viên: ' + (error.response?.data || error.message)
      );
      setShowDeleteModal(false);
    }
  };
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      toast.error(
        'Lỗi khi lưu nhân viên: ' + (error.response?.data || error.message)
      );
    }
  };
