...
const AdminDashboard = () => {
  ...
  const [maxDate, setMaxDate] = useState(new Date().toISOString().split('T')[0]); // Ngày hiện tại
  ...
  const fetchChartData = async () => {
    try {
      ...
      const filteredBySalaryVsDeduction = phieuLuongData.filter(item => {
        const date = new Date(item.ngayPhat);
        return date.getMonth() + 1 === salaryVsDeductionMonthYear.month && date.getFullYear() === salaryVsDeductionMonthYear.year;
      });
      const salaryVsDeduction = filteredBySalaryVsDeduction.reduce((acc, item) => {
        const deptName = item?.nvId?.PB_ID?.PB_TEN || 'Không xác định';
        acc[deptName] = {
      ...
      currentYearData.forEach(item => {
        ...
        if (month < selectedMonthYear.month) monthlyPayroll[month] += item.luongNhan || 0;
      });
      const labels = Array.from({ length: selectedMonthYear.month }, (_, i) => `Tháng ${i + 1}`);
  ...
  // Kiểm tra dữ liệu
  const hasDataForTotalSalary = totalSalaryData?.datasets[0]?.data.some(value => value > 0);
  const hasDataForDeductionRatio = deductionRatioData?.datasets[0]?.data.some(value => value > 0);
  const hasDataForSalaryVsDeduction = salaryVsDeductionData?.datasets[0]?.data.some(value => value > 0);
  const hasDataForMonthlySalary = monthlySalaryData?.datasets[0]?.data.some(value => value > 0);
  ...
  const handleGenerateReport = () => {
    ...
    if (new Date(endDate) > new Date(maxDate)) {
      alert('Thời gian kết thúc không được lớn hơn ngày hiện tại!');
      return;
    }

    // Lấy dữ liệu từ phieuLuongData dựa trên khoảng thời gian
    const phieuLuongResponse = fetch('http://localhost:8080/api/phieu-luong/all');
    const phieuLuongData = (await phieuLuongResponse).json();
  ...
  return (
    <Container fluid className="p-4 bg-light min-vh-100" style={{ position: 'relative', zIndex: 0 }}>
      <h1 className="text-center my-4 text-gray-800">Trang chủ - Quản lý lương</h1>
      ...
      <Row className="g-4">
          ...
          <Col md={12}>
            <Card className="shadow border-0 rounded-3 chart-card h-100">
              <Card.Header className="py-3 bg-white border-bottom d-flex justify-content-between align-items-center">
                <h6 className="m-0 stat-title text-primary">Tổng lương theo tháng</h6>
                <Form.Group controlId="yearSelectMonthly">
                  <Form.Control
                    as="select"
                    value={monthlySalaryYear}
                    onChange={e => setMonthlySalaryYear(parseInt(e.target.value))}
                    style={{ width: '100px' }}
                  >
        ...
        {hasDataForTotalSalary && (
          <Col md={12}>
            <Card className="shadow border-0 rounded-3 chart-card h-100">
              <Card.Header className="py-3 bg-white border-bottom d-flex justify-content-between align-items-center">
                <h6 className="m-0 stat-title text-primary">Tổng lương theo phòng ban</h6>
        ...
        {hasDataForDeductionRatio && (
          <Col md={6}>
            <Card className="shadow border-0 rounded-3 chart-card h-100">
              <Card.Header className="py-3 bg-white border-bottom d-flex justify-content-between align-items-center">
                <h6 className="m-0 stat-title text-primary">Tỷ lệ khấu trừ và thu nhập</h6>
        ...
        <Col md={6}>
          <Card className="shadow border-0 rounded-3 chart-card h-100">
            <Card.Header className="py-3 bg-white border-bottom">
              <h6 className="m-0 stat-title text-primary">Số nhân viên theo phòng ban</h6>
            </Card.Header>
            <Card.Body className="chart-body">
              {employeeCountData ? (
                <Doughnut
        ...
        {hasDataForSalaryVsDeduction && (
          <Col md={12}>
            <Card className="shadow border-0 rounded-3 chart-card h-100">
              <Card.Header className="py-3 bg-white border-bottom d-flex justify-content-between align-items-center">
                <h6 className="m-0 stat-title text-primary">Lương thực nhận và khấu trừ theo phòng ban</h6>
          ...
          <Card className="shadow border-0 rounded-3 chart-card h-100">
            <Card.Header className="py-3 bg-white border-bottom">
              <h6 className="m-0 stat-title text-primary">
                Tổng chi phí lương từ tháng 1 đến tháng {selectedMonthYear.month} (
      ...
      {/* Modal cho tạo báo cáo với lịch */}
        ...
        <Modal.Body>
          <Form>
            <Form.Group className="mb-3">
              ...
              >
                <option value="salaryByDepartment">Tổng lương theo phòng ban</option>
                <option value="deductionRatio">Tỷ lệ khấu trừ và thu nhập</option>
                <option value="salaryVsDeduction">Lương thực nhận và khấu trừ</option>
