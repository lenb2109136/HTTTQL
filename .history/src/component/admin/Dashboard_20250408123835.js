...
const AdminDashboard = () => {
  ...
  const fetchChartData = async () => {
    try {
      ...
      // 4. Employee Count by Department (Doughnut Chart)
      ...
      // 5. Salary vs Deduction by Department (Grouped Bar Chart)
  ...
  return (
    <Container fluid className="p-4 bg-light min-vh-100">
      <h1 className="text-center my-4 text-gray-800 fw-bold">
        ...
        {[
          {
            ...
            icon: faBuilding,
            className: 'border-left-primary text-primary',
          },
          {
            title: 'Số nhân viên',
            value: stats.nhanVienCount,
            icon: faUsers,
            className: 'border-left-success text-success',
          },
          {
            title: 'Tổng lương',
            value: stats.totalSalary.toLocaleString() + ' VND',
            icon: faMoneyBillWave,
            className: 'border-left-info text-info',
          },
          {
            title: 'Yêu cầu ứng lương',
            value: stats.ungLuongCount,
            icon: faHandHoldingUsd,
            className: 'border-left-warning text-warning',
          },
          {
            title: 'Số khiếu nại',
            value: stats.khieuNaiCount,
            icon: faExclamationTriangle,
            className: 'border-left-danger text-danger',
          },
        ].map((stat, index) => (
          <Col md={2} key={index}>
            <Card className={`shadow h-100 py-3 ${stat.className} rounded-3`}>
